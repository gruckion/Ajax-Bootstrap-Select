...
!(function ($, window) {
	...
	var AjaxBootstrapSelect = function (element, options) {
		...
		/**
		 * Count of currently displayed select items
		 * @type {int}
		 */
		this.count = 0;

		/**
		 * Total count of all select items available
		 * @type {int}
		 */
		this.total = 0;
	...
	AjaxBootstrapSelect.prototype.init = function () {
		...
		//stephen rayner - Bind this plugin's event to scroll
		this.selectpicker.$menuInner.on('scroll', function (e) {
			var query = plugin.selectpicker.$searchbox.val();
			var totalHeight = $(this)[0].scrollHeight;
			var currentHeight = $(this).scrollTop();
			var viewableHeight = $(this).outerHeight();
			//console.log("totalHeight: " + totalHeight + ", currentHeight: " + currentHeight + ", viewableHeight:" + viewableHeight);

			var cache = plugin.list.cacheGet(query);
			var count = cache ? cache.length -1 : 0;

			var total = plugin.total;
			console.log("count: " + count + ", total: " + total);

			// Don't trigger unless further than 70% towards the end
			if ((totalHeight - currentHeight) > viewableHeight
				|| typeof count == 'undefined' || typeof total == 'undefined'
				|| (count === total && total !== 0)) {
				plugin.options.ajax.data.take = plugin.options.takeIncrement + count;
				return;
			}
		...
		this.selectpicker.$searchbox.on(this.options.bindEvent, function (e) {
			...
			// Reset the take amount as any new search query should take only the initial specified amount
			plugin.options.ajax.data.take = plugin.options.takeIncrement;
	...
	var AjaxBootstrapSelectList = function (plugin) {
		...
		//TODO make this only show when searching, for now I have disabled it
	...
	AjaxBootstrapSelectList.prototype.refresh = function (triggerChange) {
		...
		//TODO scroll to correct place (without triggering feedback loop)
		//this.plugin.selectpicker.$menuInner.scrollTop(1);
	...
	AjaxBootstrapSelectRequest.prototype.process = function (data) {
		...
		//wrong approach
		//this.plugin.count = data.count;
		this.plugin.total = data.total;
		...
		if ($.isFunction(this.plugin.options.preprocessData)) {
			...
			if (typeof callbackResult !== 'undefined' && callbackResult !== null && callbackResult !== false
				&& typeof callbackResult.array && callbackResult.array !== null && callbackResult.array !== false) {
				preprocessedData = callbackResult;
			}
		}

		// Ensure the data is an array.
		if (!$.isArray(preprocessedData.array)) {
		...
		l = preprocessedData.array.length;
		for (i = 0; i < l; i++) {
			item = preprocessedData.array[i];
		...
		if ($.isFunction(this.plugin.options.processData)) {
			...
			if (typeof callbackResult !== 'undefined' && callbackResult !== null && callbackResult !== false
				&& typeof callbackResult.array && callbackResult.array !== null && callbackResult.array !== false) {
				if ($.isArray(callbackResult.array)) {
	...
	$.fn.ajaxSelectPicker.defaults = {
		...
		/**
		 * @member $.fn.ajaxSelectPicker.defaults
		 * @cfg {Int} takeIncrement = 50
		 * @markdown
		 * Specify the number of entries in the buffer request
		 */
		takeIncrement: 50,
